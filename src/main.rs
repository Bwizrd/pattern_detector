...
mod multi_backtest_handler; // You have this for /multi-symbol-backtest
mod patterns;
...
mod optimize_handler; // Contains both /optimize-parameters and the new /portfolio-meta-backtest
...
async fn main() -> std::io::Result<()> {
    ...
    println!("  POST /multi-symbol-backtest"); // This one is important
    println!("  POST /optimize-parameters");   // This one is important
    println!("  POST /portfolio-meta-backtest"); // NEW ONE
    ...
    HttpServer::new(move || {
        ...
        App::new()
            ...
            .route("/multi-symbol-backtest", web::post().to(multi_backtest_handler::run_multi_symbol_backtest)) // Existing
            .route("/optimize-parameters", web::post().to(optimize_handler::run_parameter_optimization)) // Existing (now calls refactored logic)
            .route("/portfolio-meta-backtest", web::post().to(optimize_handler::run_portfolio_meta_optimized_backtest)) // <<< NEW ENDPOINT ADDED HERE
