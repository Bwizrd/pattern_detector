...
use crate::detect::{CandleData, CoreError};
...
impl ZoneDetectionEngine {
    ...
    fn extract_and_enrich_zones(
        ...
        if let Some(data) = detection_results.get("data").and_then(|d| d.get("price")) {
            ...
            if let Some(supply_zones_data) = data.get("supply_zones")
            ...
            if let Some(demand_zones_data) = data.get("demand_zones")
        ...
        if let Some(max_touches) = max_touch_count {
            ...
            final_supply_zones.retain(|zone| {
                zone.touch_count.map_or(true, |tc| tc <= max_touches)
            });
            
            final_demand_zones.retain(|zone| {
                zone.touch_count.map_or(true, |tc| tc <= max_touches)
            });
            
            debug!(
                "[ZoneEngine] Touch count filter (max: {}): Supply {} -> {}, Demand {} -> {}",
                max_touches, initial_supply, final_supply_zones.len(),
                initial_demand, final_demand_zones.len()
            );
        }
    ...
    fn enrich_zones_with_activity(
        ...
        debug!("[ZoneEngine] Enriching {} {} zones for {}/{}", 
               raw_zones.len(), zone_type_str, symbol, timeframe);
            ...
            let mut enriched_zone = match crate::types::deserialize_raw_zone_value(zone_json) {
                Ok(mut z) => {
                    ...
                    if z.zone_id.is_none() {
                        let id = crate::detect::generate_deterministic_zone_id(
                            symbol, timeframe, 
                    ...
                    if z.symbol.is_none() {
                        z.symbol = Some(symbol.to_string());
                    }
                ...
                Err(e) => {
                    warn!("[ZoneEngine] Failed to deserialize zone #{}: {}", zone_counter + 1, e);
                    continue;
                }
            };
    ...
    pub fn detect_zones_batch(&self, requests: Vec<ZoneDetectionRequest>) -> Vec<Result<ZoneDetectionResult, CoreError>> {
        requests.into_iter()
                ...
                for result in rdr.records() {
                    match result {
                        Ok(record) => {
                            ...
                            if record.len() > std::cmp::max(t_idx, std::cmp::max(o_idx, std::cmp::max(h_idx, std::cmp::max(l_idx, std::cmp::max(c_idx, v_idx))))) {
