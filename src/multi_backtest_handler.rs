...
pub async fn run_multi_symbol_backtest(
    ...
    let org = match std::env::var("INFLUXDB_ORG") {
        Ok(val) => val, Err(e) => { error!("INFLUXDB_ORG not set: {}", e); return Ok(HttpResponse::InternalServerError().json("Server configuration error: INFLUXDB_ORG missing.")); }
    };
    let token = match std::env::var("INFLUXDB_TOKEN") {
        Ok(val) => val, Err(e) => { error!("INFLUXDB_TOKEN not set: {}", e); return Ok(HttpResponse::InternalServerError().json("Server configuration error: INFLUXDB_TOKEN missing.")); }
    };
    let bucket = match std::env::var("INFLUXDB_BUCKET") {
        Ok(val) => val, Err(e) => { error!("INFLUXDB_BUCKET not set: {}", e); return Ok(HttpResponse::InternalServerError().json("Server configuration error: INFLUXDB_BUCKET missing.")); }
    };

    let pattern_timeframes = vec!["30m", "1h"];
    ...
    let mut pnl_by_day_pips: HashMap<String, f64> = HashMap::new();
    ...
    let overall_summary = OverallBacktestSummary {
        ...
        total_pnl_pips: overall_total_pnl_pips,
        overall_win_rate_percent: overall_win_rate,
        overall_profit_factor: overall_profit_factor_val,
        trades_by_symbol_percent,
        trades_by_timeframe_percent,
        trades_by_day_of_week_percent,
        pnl_by_day_of_week_pips: pnl_by_day_pips,
        trades_by_hour_of_day_percent,
        pnl_by_hour_of_day_pips: pnl_by_hour_pips,
    };
