...
pub fn generate_report(
    ...
    write_html_header(&mut html_file, &config.symbol, total_days, &start_datetime, &end_datetime, results.len())?;
...
<body>
    ...
    </div>"#, symbol, total_days, start_datetime.format("%Y-%m-%d"), end_datetime.format("%Y-%m-%d"), total_results)?;
    
    Ok(())
}

fn write_profit_factor_table(file: &mut File, results: &[OptimizationResult]) -> Result<(), Box<dyn std::error::Error>> {
    writeln!(file, r#"
...
fn write_net_pips_table(file: &mut File, results: &[OptimizationResult]) -> Result<(), Box<dyn std::error::Error>> {
    writeln!(file, r#"
...
fn print_console_summary(
    ...
    if !pf_sorted.is_empty() && !pips_sorted.is_empty() {
        ...
        println!("  TP/SL: {:.1}/{:.1} pips", pf_sorted[0].take_profit_pips, pf_sorted[0].stop_loss_pips);
        ...
        println!("  TP/SL: {:.1}/{:.1} pips", pips_sorted[0].take_profit_pips, pips_sorted[0].stop_loss_pips);
